...
const websocketRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/docs", {
    schema: {
      description: "WebSocket API Documentation - This endpoint documents the WebSocket message types but does not accept HTTP requests. WebSocket connections should be made to the /ws endpoint.",
      tags: ["WebSocket API"]
    }
  }, async (request, reply) => {
    return reply.send({
      message: "This endpoint is for documentation only. Use WebSocket connection to /ws for real-time communication.",
      websocket_url: "/ws",
      client_messages: [
        "join_strict", "join_loose", "cancel", "signal", 
        "call_end", "feedback", "heartbeat", "reconnect"
      ],
      server_messages: [
        "match_found", "waiting", "cancelled", "error", 
        "feedback_received", "pong"
      ]
    });
  });

  // Example WebSocket flow documentation
  app.get("/websocket/examples", {
    schema: {
    ...
    return reply.send({
      examples: [
        ...
        },
        {
          ...
          description: "User cancels match request and submits feedback after call",
          flow: [
            {
              from: "Client",
              to: "Server",
